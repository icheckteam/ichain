...
const (
	defaultIchainHouseName = "IchainHouse"
	defaultConfigBaseDir   = ".ichaind"
	flagClearingHouseName  = "ichain-house-name"
)
...
func initCommand(logger log.Logger) *cobra.Command {
	...
	cmd.Flags().String(flagClearingHouseName, defaultIchainHouseName, "Ichain House name")
...
func main() {
	...
	initCmd.Flags().String(flagClearingHouseName, defaultIchainHouseName, "Ichain House name")
	...
	executor := cli.PrepareBaseCmd(ichaindCmd, "IC", rootDir)
	executor.Execute()
}
...
func generateApp(rootDir string, logger log.Logger) (abci.Application, error) {
	db, err := dbm.NewGoLevelDB("ichain", rootDir)
	if err != nil {
		return nil, err
	}
